    ...
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="glass-card relative overflow-hidden p-6 lg:col-span-2">
          ...
          <div className="relative z-10 space-y-3">
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white">User base snapshot</h2>
            <p className="text-sm text-slate-600 dark:text-slate-300">{users.length} active profiles across the digital banking ecosystem.</p>
      ...
      <section className="glass-card overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-slate-200/60 px-6 py-5 sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300">Audit-ready overview of platform access.</p>
          </div>
          <span className="rounded-full border border-slate-200 bg-white/80 px-4 py-1 text-xs font-semibold uppercase tracking-wider text-slate-600 dark:border-white/20 dark:bg-white/5 dark:text-slate-200">Updated {new Date().toLocaleTimeString()}</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-white/10 text-sm text-slate-200">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {users.length === 0 && (
                <tr>
                  <td colSpan="4" className="px-6 py-8 text-center text-slate-500 dark:text-slate-300">No users available.</td>
                </tr>
              )}
              {users.map((u) => (
                <tr key={u.id || u._id} className="transition hover:bg-slate-100/80 dark:hover:bg-white/5">
